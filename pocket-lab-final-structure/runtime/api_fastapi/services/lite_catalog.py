...
def _app_payload(app: dict[str, Any], access: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "actions": actions,
        "runtime": {
            "route": PHOTOPRISM_ROUTE,
            "url": open_url,
            "health": runtime_health,
            "version": (app.get("runtime") or {}).get("version") if isinstance(app.get("runtime"), dict) else None,
            "process": runtime_evidence.get("process") if isinstance(runtime_evidence.get("process"), dict) else {"name": PHOTOPRISM_PROCESS, "status": "unknown"},
            "installation_state": runtime_evidence.get("installation_state"),
            "process_status": (runtime_evidence.get("process") or {}).get("status"),
            "reachable": bool(runtime_evidence.get("reachable")),
            "running": bool(runtime_evidence.get("running")),
            "state_conflict": bool(runtime_evidence.get("state_conflict")),
            "evidence_quality": runtime_evidence.get("evidence_quality") or "unknown",
            "authoritative_source": runtime_evidence.get("authoritative_source") or "runtime_evidence",
            "evidence": runtime_evidence.get("evidence") or {},
        },
