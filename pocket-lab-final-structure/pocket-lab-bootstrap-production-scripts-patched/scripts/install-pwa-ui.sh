...
allowed_hosts={'github.com','api.github.com','objects.githubusercontent.com','release-assets.githubusercontent.com','github-releases.githubusercontent.com'}

def trusted_download_url(value):
    parsed=urllib.parse.urlparse(str(value or ''))
    host=(parsed.hostname or '').lower()
    allowed=host in allowed_hosts or host.endswith('.githubusercontent.com')
    return bool(parsed.scheme == 'https' and not parsed.username and not parsed.password and allowed)
...
def embedded_assets(release):
    ...
    for item in release.get('assets') or []:
        ...
        if not trusted_download_url(url):
            duplicate=True
        assets[name]=url
    complete=not duplicate and set(assets) == required
    return assets if complete else {}
