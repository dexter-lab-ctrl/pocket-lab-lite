...
export const liteApi = {
  ...
  telemetryThresholds: conditionalGet('/api/lite/system/telemetry-thresholds'),
  storagePressure: conditionalGet('/api/lite/system/storage-pressure'),
  sqliteHealth: conditionalGet('/api/lite/system/sqlite-health'),
  activitySummary: conditionalGet('/api/lite/system/activity-summary'),
  ...
  ownerClaimStatus: () => readJson('/api/lite/identity/owner-claim/status'),
  consumeOwnerClaim: (claim) => postJson('/api/lite/identity/owner-claim/consume', { claim }),
  ownerClaimPasskeyOptions: (payload = {}) => postJson('/api/lite/identity/owner-claim/passkey/options', payload),
  verifyOwnerClaimPasskey: (payload = {}) => postJson('/api/lite/identity/owner-claim/passkey/verify', payload).then(rememberIdentityCsrf),
  passkeyLoginOptions: () => postJson('/api/lite/identity/passkeys/login/options', {}),
  verifyPasskeyLogin: (payload = {}) => postJson('/api/lite/identity/passkeys/login/verify', payload).then(rememberIdentityCsrf),
  passkeyRegistrationOptions: () => postJson('/api/lite/identity/passkeys/registration/options', {}),
  verifyPasskeyRegistration: (payload = {}) => postJson('/api/lite/identity/passkeys/registration/verify', payload),
  passkeyStepUpOptions: (purpose = 'identity.passkey.revoke') => postJson('/api/lite/identity/step-up/options', { purpose }),
  verifyPasskeyStepUp: (payload = {}) => postJson('/api/lite/identity/step-up/verify', payload),
  renameIdentityPasskey: (credentialId, friendlyName) => putJson(`/api/lite/identity/passkeys/${encodeURIComponent(credentialId || '')}`, { friendly_name: friendlyName }),
  revokeIdentityPasskey: (credentialId) => deleteJson(`/api/lite/identity/passkeys/${encodeURIComponent(credentialId || '')}`),
  enterpriseIdentity: () => readJson('/api/lite/enterprise/identity'),
  setEnterpriseMode: (enabled) => putJson('/api/lite/enterprise/identity/mode', { enabled }),
  enterpriseMembers: () => readJson('/api/lite/enterprise/identity/members'),
  updateEnterpriseMember: (humanId, payload = {}) => putJson(`/api/lite/enterprise/identity/members/${encodeURIComponent(humanId || '')}`, payload),
  enterpriseRulesHealth: () => readJson('/api/lite/enterprise/rules/health'),
  enterpriseRuleRevisions: () => readJson('/api/lite/enterprise/rules/revisions'),
  enterpriseRulesAnalysis: (revisionId = '') => readJson(`/api/lite/enterprise/rules/analysis${revisionId ? `?revision_id=${encodeURIComponent(revisionId)}` : ''}`),
  enterpriseRuleDecisions: (query = '') => readJson(`/api/lite/enterprise/rules/decisions${query ? `?${query}` : ''}`),
  enterpriseRuleDecision: (decisionId) => readJson(`/api/lite/enterprise/rules/decisions/${encodeURIComponent(decisionId || '')}`),
  simulateEnterpriseRule: (payload = {}) => postJson('/api/lite/enterprise/rules/simulations', payload),
  enterpriseRuleApprovals: () => readJson('/api/lite/enterprise/rules/approvals'),
  enterpriseRuleApproval: (approvalId) => readJson(`/api/lite/enterprise/rules/approvals/${encodeURIComponent(approvalId || '')}`),
  transitionEnterpriseRuleApproval: (approvalId, action) => postJson(`/api/lite/enterprise/rules/approvals/${encodeURIComponent(approvalId || '')}`, { action }),
  enterpriseRuleExceptions: () => readJson('/api/lite/enterprise/rules/exceptions'),
  createEnterpriseRuleException: (payload = {}) => postJson('/api/lite/enterprise/rules/exceptions', payload),
  revokeEnterpriseRuleException: (exceptionId) => postJson(`/api/lite/enterprise/rules/exceptions/${encodeURIComponent(exceptionId || '')}/revoke`, {}),
  ...
  policyTemplates: () => readJson('/api/lite/policy/templates'),
  policyDecision: (decisionId) => readJson(`/api/lite/policy/decisions/${encodeURIComponent(decisionId || '')}`),
