...
export function selectDeviceSystemProfileView(device = {}) {
  ...
  return {
    ...
    architecture: safeString(profile.architecture || profile.architecture_family || device.architecture || device.architecture_family || ''),
    architecture_raw: safeString(profile.architecture_raw || profile.android_abi || device.architecture || ''),
    architecture_family: safeString(profile.architecture_family || profile.architecture || device.architecture_family || ''),
    android_abi: safeString(profile.android_abi || profile.architecture_raw || device.architecture || ''),
    kernel: safeString(profile.kernel || ''),
    runtime_type: safeString(profile.runtime_type || device.runtime_type || ''),
...
export function selectLiteDeviceCard(device = {}) {
  ...
  const output = {
    ...
    command_delivery_status: normalizeDeviceStatus(device.command_delivery_status || device.delivery_status || ''),
    ...
    restart_agent_assessment: isObject(device.restart_agent_assessment) ? copySafeKeys(device.restart_agent_assessment, ['allowed', 'reason_code', 'summary', 'command_deliverable', 'supervisor_fresh', 'agent_state']) : null,
    runtime_services: Array.isArray(device.runtime_services) ? device.runtime_services.slice(0, 8).map((item) => copySafeKeys(item, ['service_id', 'label', 'manager', 'state', 'reported_at', 'freshness', 'restart_supported', 'restart_reason'])) : [],
    can_restart_agent: Boolean(device.restart_agent_assessment?.allowed === true),
...
export function selectFleetDevicesView(payload = {}) {
  ...
  return withSnapshotMeta(payload, {
    ...
    online_count: devices.filter((device) => normalizeDeviceStatus(device.connection) === 'online').length,
