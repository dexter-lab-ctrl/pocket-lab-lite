...
describe('Lite Home presentation model', () => {
  ...
  it('normalizes service tones and current resource thresholds deterministically', () => {
    ...
    const overview = buildLiteHomeOverview({
      telemetry: {
        pocketlab_workload_cpu_percent: 94,
        pocketlab_workload_process_count: 4,
        cpu_temp_c: 60,
        free_space_mb: 400,
        total_space_mb: 10_000,
        memory_usage_mb: 7_500,
    ...
    expect(overview.resources.find((item) => item.key === 'device-health')).toMatchObject({
      value: '0.5 GB free / 7.8 GB',
      tone: 'danger',
      note: 'Pocket Lab CPU 94% · 60°C',
    });
    expect(overview.resources.find((item) => item.key === 'storage')).toMatchObject({
      value: '0.4 GB free / 9.8 GB',
      tone: 'danger',
  ...
  it('uses per-card semantic fallbacks without hiding valid telemetry', () => {
    const overview = buildLiteHomeOverview({
      ...
      telemetry: {
        free_space_mb: 135569,
        total_space_mb: 228000,
        pocketlab_workload_cpu_percent: 0,
        pocketlab_workload_process_count: 3,
        cpu_temp_c: 35.2,
        memory_usage_mb: 4054,
        memory_total_mb: 7900,
        memory_free_mb: 3846,
      },
    ...
    expect(overview.resources.find((item) => item.key === 'device-health')).toMatchObject({ value: '3.8 GB free / 7.7 GB', tone: 'ready', note: 'Pocket Lab CPU 0% · 35°C' });
    expect(overview.resources.find((item) => item.key === 'storage')).toMatchObject({ value: '132 GB free / 223 GB', tone: 'ready', note: '59% available for apps and backups' });
