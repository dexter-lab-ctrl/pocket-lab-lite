...
def _command(database: Path, command_id: str) -> dict:
    ...
    try:
        row = conn.execute(
            "SELECT * FROM command_lifecycle WHERE command_id=?", (command_id,)
        ).fetchone()
        return dict(row)
    finally:
        conn.close()
...
def test_legacy_missing_target_becomes_undeliverable_and_propagates(tmp_path, monkeypatch):
    ...
    try:
        revisions = dict(conn.execute(
            "SELECT domain,revision FROM domain_revisions WHERE domain IN ('commands','audit')"
        ))
...
def test_recent_queued_command_remains_nonterminal(tmp_path, monkeypatch):
    ...
    result = ControlPlaneProjectionStore().reconcile_command_lifecycle(
        now_epoch_ms=10_000_000,
        legacy_max_age_seconds=3600,
        missing_target_grace_seconds=300,
    )
...
def test_known_offline_device_respects_grace_then_becomes_undeliverable(tmp_path, monkeypatch):
    ...
    _insert_command(
        ...
        entity_id="offline-known",
        updated_at_epoch_ms=9_900_000,
    )
    store = ControlPlaneProjectionStore()
    recent = store.reconcile_command_lifecycle(
        now_epoch_ms=10_000_000, legacy_max_age_seconds=3600
    )
    assert recent["reconciled_count"] == 0

    old = store.reconcile_command_lifecycle(
        now_epoch_ms=20_000_000, legacy_max_age_seconds=60
    )
...
def test_deadline_uses_schema_valid_timed_out_status(tmp_path, monkeypatch):
    ...
    _insert_command(
        database,
        command_id="deadline",
        deadline_at="1970-01-01T00:00:05Z",
...
def test_terminal_command_never_regresses_and_reconcile_is_idempotent(tmp_path, monkeypatch):
    ...
    try:
        conn.execute(
            "UPDATE command_lifecycle SET lifecycle_stage='terminal',terminal_at='2026-07-01T00:00:00Z' WHERE command_id='terminal'"
        )
        conn.commit()
    finally:
        conn.close()
...
def test_unacknowledged_failure_requires_attention_and_acknowledgement_clears_it(tmp_path, monkeypatch):
    ...
    store.record_command(
        command_id="attention-command",
        subject="agent.restart",
        status="undeliverable",
        entity_type="device",
        entity_id="offline-device",
        summary="Could not deliver restart.",
    )
