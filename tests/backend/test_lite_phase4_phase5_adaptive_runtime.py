...
def test_adaptive_cadence_slows_after_unchanged_and_accelerates_for_transition():
    ...
    for _ in range(5):
        controller.record_result(domain, cpu_ms=1, wall_ms=2, queue_wait_ms=0, outcome="unchanged", changed=False)
    ...
    controller.record_result(domain, cpu_ms=2, wall_ms=3, queue_wait_ms=0, outcome="committed", changed=True, active_transition=True)
...
def test_adaptive_cpu_budget_queue_pressure_and_critical_reserve():
    ...
    controller = AdaptiveRuntimeController()
    domain = "apps.catalog"
    controller._policies[domain] = AdaptivePolicy(active_interval_seconds=1, stable_interval_seconds=10, max_interval_seconds=30, transition_hold_seconds=2, short_cpu_budget_ms=5, medium_cpu_budget_ms=5, long_cpu_budget_ms=5, payload_budget_bytes=64 * 1024, allocation_budget_bytes=256 * 1024, serialization_budget_ms=50, critical=False)
    controller.record_result(domain, cpu_ms=6, wall_ms=6, queue_wait_ms=0, outcome="committed", changed=True)
    deferred = controller.decide(domain, priority=60, work_class="io", optional=True, queue_depth=0, queue_age_ms=0, active_count=0, capacity=1)
    assert deferred.accepted is False
    assert deferred.reason == "cpu_budget_exhausted"
    assert deferred.retry_after_ms > 0

    critical = controller.decide(domain, priority=10, work_class="critical", optional=False, queue_depth=controller.queue_depth_critical + 5, queue_age_ms=controller.queue_lag_critical_ms + 1, active_count=2, capacity=1, event_loop_lag_ms=controller.event_loop_critical_ms + 1)
    assert critical.accepted is True
    assert critical.load_state == "critical"

    queue_deferred = controller.decide("recovery.details", priority=80, work_class="io", optional=True, queue_depth=controller.queue_depth_warning, queue_age_ms=0, active_count=0, capacity=2)
...
def test_payload_assessment_is_deterministic_and_enforces_budgets():
    ...
    controller._policies[domain] = AdaptivePolicy(active_interval_seconds=1, stable_interval_seconds=10, max_interval_seconds=30, transition_hold_seconds=2, short_cpu_budget_ms=100, medium_cpu_budget_ms=100, long_cpu_budget_ms=100, payload_budget_bytes=100, allocation_budget_bytes=1024 * 1024, serialization_budget_ms=500, critical=False)
...
def test_compact_event_envelope_is_additive_sanitized_and_size_bounded(monkeypatch):
    ...
    event = bus.envelope("pocketlab.events.security.scan", "security.scan.updated", {"domain": "security.progress", "entity_id": "run-1", "generation": 4, "status": "running", "reason_code": "worker_claimed", "password": "must-not-be-promoted", "metadata": {"safe": True}}, trace_id="trace-1")
    ...
    with pytest.raises(ValueError, match="transport budget"):
        bus._encode_event("pocketlab.events.security.scan", bus.envelope("pocketlab.events.security.scan", "security.scan.updated", {"metadata": "x" * (20 * 1024)}))
...
def test_bounded_process_runtime_caps_output_and_defers_at_capacity(tmp_path, monkeypatch):
    ...
    class FakeProcess:
        pid = 123456789
        returncode = 0
        def communicate(self, timeout=None): return ("x" * (70 * 1024), "")
    ...
    result = runtime.run(["fake", "--safe"], cwd=tmp_path, timeout=5, workload="security.trivy", redact=lambda value: value, popen_factory=lambda *args, **kwargs: FakeProcess())
    ...
    try:
        deferred = runtime.run(["fake"], cwd=tmp_path, timeout=5, workload="maintenance.integrity", redact=lambda value: value, popen_factory=lambda *args, **kwargs: FakeProcess())
    finally:
...
def test_core_supervisor_restart_budget_is_bounded(tmp_path, monkeypatch):
    ...
    allowed, reason, retry_after, generation = supervisor._restart_admission("pocket-worker", "restart:pocket-worker")
...
def test_prepared_read_and_frontend_expose_truthful_capacity_state():
    ...
    prepared = PreparedRead(payload={"status": "healthy"}, etag='"phase45"', source_revision=4, projection_age_ms=100, read_degraded=True, refresh_pending=True, timing={"sqlite_query_ms": 0.4}, retry_after_seconds=5, retry_after_ms=5000, degraded_reason="queue_pressure", data_source="last_known_good", load_state="elevated")
...
def test_adaptive_gate_analyzer_distinguishes_failures_and_unsupported_metrics():
    ...
    spec.loader.exec_module(module)
    class Args:
        minimum_samples = 5; queue_depth_budget = 12; cpu_p99_budget_ms = 750; wall_p99_budget_ms = 8000; queue_wait_p99_budget_ms = 20000; max_exhaustion_ratio = 0.5; api_p95_budget_ms = 750; api_p99_budget_ms = 2000; event_loop_p95_budget_ms = 150; event_loop_p99_budget_ms = 400; rss_growth_budget_bytes = 64 * 1024 * 1024; peak_rss_budget_bytes = 768 * 1024 * 1024
    sample = {"http": {"ok": True, "duration_ms": 10}, "scheduler": {"queued_domains": 0}, "event_loop": {"latest_lag_ms": 1}, "adaptive": {"event_payloads": {"oversize_rejections": 0}, "domains": {"system.health": {"cpu_budget_exhausted": False, "load_state": "normal", "cpu_ms": {"p99": 2}, "wall_ms": {"p99": 3}, "queue_wait_ms": {"p99": 1}, "payload_bytes": {"max": 100}, "allocation_bytes": {"max": 200}, "serialization_ms": {"max": 1}, "payload_budget_bytes": 1000, "allocation_budget_bytes": 2000, "serialization_budget_ms": 10}}}, "process_runtime": {"subprocess_count": 0, "subprocess_limit": 1, "memory_rss_bytes": None, "workloads": {}}}
    ...
    overloaded = json.loads(json.dumps(sample)); overloaded["scheduler"]["queued_domains"] = 99
...
def test_worker_registers_core_projection_mailbox_domains_before_consumption() -> None:
    ...
    shared = (root / "pocket-lab-final-structure/runtime/api_fastapi/services/lite_core_projections.py").read_text(encoding="utf-8")
    assert "await asyncio.to_thread(lite_core_projections.register_jobs)" in worker
    assert worker.index("lite_core_projections.register_jobs") < worker.index("PROJECTION_SCHEDULER.consume_dirty_signals")
...
def test_worker_projection_mailbox_loop_imports_monotonic_clock() -> None:
    root = Path(__file__).resolve().parents[2]
    worker_path = root / "pocket-lab-final-structure/runtime/workers/pocketlab_worker.py"
...
def test_projection_scheduler_queue_depth_reconciles_stale_flags() -> None:
    ...
    scheduler.register(ProjectionJob(domain="test.queue.truth", builder=lambda: {}, projector=lambda _payload: 1, priority=50, work_class="io", deadline_seconds=1.0))
    with scheduler._condition:
        state = scheduler._states["test.queue.truth"]; state.queued = True; state.dirty = False; scheduler._heap.clear()
...
def test_projection_scheduler_ready_depth_excludes_future_entries() -> None:
    ...
    from api_fastapi.services.projection_scheduler import ProjectionJob, ProjectionScheduler
    scheduler = ProjectionScheduler()
    scheduler.register(ProjectionJob(domain="test.queue.future", builder=lambda: {}, projector=lambda _payload: 1, priority=50, work_class="io", deadline_seconds=1.0))
    with scheduler._condition:
        state = scheduler._states["test.queue.future"]; state.generation += 1; state.dirty = True; state.not_before_at = time.monotonic() + 60.0; scheduler._enqueue_locked("test.queue.future", state)
...
def test_runtime_snapshot_hot_path_is_preencoded_revisioned_and_nonblocking() -> None:
    root = Path(__file__).resolve().parents[2]
    scheduler = (root / "pocket-lab-final-structure/runtime/api_fastapi/services/projection_scheduler.py").read_text(encoding="utf-8")
    runtime = (root / "pocket-lab-final-structure/runtime/api_fastapi/services/runtime_snapshot_store.py").read_text(encoding="utf-8")
    diagnostics = (root / "pocket-lab-final-structure/runtime/api_fastapi/services/runtime_diagnostics.py").read_text(encoding="utf-8")
    router = (root / "pocket-lab-final-structure/runtime/api_fastapi/routers/lite.py").read_text(encoding="utf-8")
    ...
    assert "json.loads(row[\"payload_json\"])" not in runtime.split("def encoded_runtime_response", 1)[1]
    ...
    assert "RUNTIME_DIAGNOSTICS.snapshot()" not in router.split('@router.get("/diagnostics/runtime")', 1)[1].split('@router.get("/diagnostics/runtime/full")', 1)[0]
...
def test_worker_registry_covers_ui_critical_catalog_domains_and_is_authoritative() -> None:
    ...
    assert {"apps.catalog", "apps.actions:photoprism", "apps.lifecycle", "fleet.summary", "recovery.summary", "recovery.details"}.issubset(lite_core_projections.CORE_PROJECTION_DOMAINS)
    assert "system.status" in lite_core_projections.UI_CRITICAL_BOOTSTRAP_DOMAINS
    assert {"system.health", "system.fleet_probe", "system.nats_remote", "system.telemetry_thresholds", "system.storage_pressure", "system.sqlite_health", "system.activity_current", "system.activity_history"}.issubset(lite_core_projections.UI_CRITICAL_BOOTSTRAP_DOMAINS)
    root = Path(__file__).resolve().parents[2]
    worker = (root / "pocket-lab-final-structure/runtime/workers/pocketlab_worker.py").read_text(encoding="utf-8")
    diagnostics = (root / "pocket-lab-final-structure/runtime/api_fastapi/routers/lite.py").read_text(encoding="utf-8")
