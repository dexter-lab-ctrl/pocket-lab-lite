#!/usr/bin/env bash
# Phase 5 Gate 4.3: operator-assisted Android background and resume.
set -Eeuo pipefail
stage='android-lifecycle'
[[ "$(long_gate_resume_stage_status "$LONG_GATE_GATE_ID" "$stage")" == 'passed' ]] && return 0
long_gate_stage_begin "$LONG_GATE_GATE_ID" "$stage" 1
args=(
  android-resume --repo-root "$LONG_GATE_REPO_ROOT" --run-dir "$LONG_GATE_RUN_DIR" --run-id "$LONG_GATE_RUN_ID"
  ...
  --submission-timeout-seconds "$LONG_GATE_SUBMISSION_TIMEOUT_SECONDS" --scenario "$LONG_GATE_ANDROID_SCENARIO"
  --operator-timeout-seconds "$LONG_GATE_ANDROID_OPERATOR_TIMEOUT_SECONDS"
  --frontend-report-timeout-seconds "$LONG_GATE_FRONTEND_REPORT_TIMEOUT_SECONDS" --resume-cycles "$LONG_GATE_ANDROID_RESUME_CYCLES"
)
[[ "$LONG_GATE_ANDROID_AUTO_CONFIRM" == '1' ]] && args+=(--auto-confirm-operator)
...
reason="$(long_gate_gate_failure_reason "$LONG_GATE_GATE_ID")"; [[ -n "$reason" ]] || reason="Android lifecycle gate failed with exit code $rc."
