#!/usr/bin/env bash
# Phase 5 Gate 3.1: deterministic submission timeout recovery.
set -Eeuo pipefail

stage='controlled-submission-recovery'
...
set +e
"$LONG_GATE_PYTHON" "$LONG_GATE_GROUP3_TOOL" submission-recovery \
  ...
  --client-timeout-seconds "$LONG_GATE_CLIENT_TIMEOUT_SECONDS" \
  --response-delay-ms "$LONG_GATE_RESPONSE_DELAY_MS" \
  --discovery-timeout-seconds "$LONG_GATE_DISCOVERY_TIMEOUT_SECONDS" \
...
[[ -n "$reason" ]] || reason="Submission timeout recovery gate failed with exit code $rc."
