#!/usr/bin/env bash
# Phase 5 Gate 3.2: controlled NATS restart recovery.
set -Eeuo pipefail

stage='controlled-nats-recovery'
...
set +e
"$LONG_GATE_PYTHON" "$LONG_GATE_GROUP3_TOOL" nats-restart \
...
[[ -n "$reason" ]] || reason="NATS restart recovery gate failed with exit code $rc."
