...
def production_outputs(source_commit: str) -> dict[Path, str]:
    ...
    outputs[PROD / "recovery.md"] = header("Backup and restore", "Production", source_commit, "Recovery is backend/worker-owned. The UI can request a backup, verification, restore preview, and confirmed restore only through supported FastAPI endpoints.") + "Restore requires explicit confirmation, a pre-restore checkpoint, and post-restore health validation. Saved-state display never fakes action success. Backup destinations are backend-owned Server Phone locations identified by opaque IDs; history remains bound to the repository that created each restore point, and unavailable external locations fail closed. The Android system folder picker is not implemented, so the PWA presents only backend-discovered candidates and never accepts a raw browser path.\n"
    ...
    outputs[PROD / "recovery.md"] = user_guide(source_commit, "Backup & Restore", "Backup & Restore manages a guarded sequence: Backup → Verified → Preview → Checkpoint → Restored. A backup is not automatically verified, and saved state never authorizes a write.", ["Backup readiness, history/restore points, app backups, targets, evidence and copy controls", "Backend-owned Server Phone backup location, location health, and immutable restore-point binding", "Backup, verification, non-mutating preview, explicit restore confirmation", "Maintenance lock, unresolved-restore guard, active restore, historical preview, and projection reconciliation states"], [("Backup Now", "Recovery", "Requests a backup in the selected backend location", "When target, location, and guards allow", "Backend/worker creates a backup", "Backup entry with location", "Target, location, lock, or stale projection guard"), ("Choose backup location", "Recovery", "Selects or discovers a validated Server Phone location", "When no Recovery mutation is running", "Shows backend-discovered candidates and bounded health", "Selected location", "Picker unavailable, location missing, read-only, or low space"), ("Verify Backup", "Backup detail", "Requests verification from the restore point's recorded repository", "When a backup and its location are available", "Produces verification evidence", "Verified state", "Backup unavailable, repository mismatch, or verification failure"), ("Preview Restore", "Recovery", "Requests a non-mutating preview from the recorded source location", "When a restore point exists and its repository is available", "Shows planned restore evidence", "Preview result", "Historical/projection state unavailable"), ("Restore Latest", "Recovery", "Requests confirmed restore to This Server Phone", "After confirmation and guards", "Checkpoint → restore → service restart → health validation", "Restored and health evidence", "Location, maintenance, unresolved guard, active restore, or validation failure"), ("Manage / details / copy evidence", "Recovery cards", "Opens details or copies bounded evidence", "When an item exists", "Shows retained status", "No safe evidence available")], ["Create a backup, verify it, and use Preview Restore before a confirmed restore.", "Choose a backend-discovered location when needed; history keeps older restore points tied to their original location.", "Resolve stale/saved projection status or reconnect an unavailable location before retrying a write."], "Active restore, maintenance locks, unresolved guards, and stale projection write blocking are safety controls, not successful completion.", "Restore is backend/worker-owned, requires confirmation and a pre-restore checkpoint, and ends with post-restore health validation. The browser cannot write backup storage directly or submit a trusted raw path.", "[Backup & Restore Feature Journey](../enterprise/journeys/recovery.md)", "[Backup/recovery architecture](architecture/backup-recovery.md) · [Security boundaries](security-boundaries.md)")
