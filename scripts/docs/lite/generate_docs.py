...
def frontmatter(audience: str, source_commit: str, fingerprint: str) -> str:
    return (
        "---\n"
        "generated: true\n"
...
def header(title: str, audience: str, source_commit: str, intro: str) -> str:
    return frontmatter(audience, source_commit, aggregate_fingerprint()) + f"# {title}\n\n{intro}\n\n"
...
"""
    ...
    outputs[DEV / "reason-codes.md"] = header("Reason-code registry", "Development", source_commit, "Reason codes are scanned from backend source to keep Storybook, Playwright, docs, and frontend presentation aligned with backend truth.") + f"## Detected codes ({len(reasons)})\n\n{markdown_list(reasons)}\n"
...
def production_outputs(source_commit: str) -> dict[Path, str]:
    ...
    outputs[PROD / "limitations.md"] = header("Current limitations", "Production", source_commit, "Only verified deployable behavior is listed.") + "- Identity role-aware governance is not presented as complete.\n- Rules approval/execution workflows are not presented as complete.\n- Android performance claims require server-phone evidence; desktop Lighthouse alone is insufficient.\n- Live browser and long-duration qualification require a running isolated stack and explicit operator action.\n"
