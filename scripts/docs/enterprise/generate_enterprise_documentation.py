...
def build() -> tuple[dict[Path,str], dict[str,Any]]:
    ...
    outputs[DOC/"reference/dependency-health.md"]=doc_page("Dependency Health","Graphviz dependency-health visualization derived from canonical metadata and promoted evidence.","development","# Dependency Health\n\nA domain state never silently promotes a child dependency to healthy. Unknown evidence remains unvalidated.\n\n<div class=\"pl-generated-diagram\">\n\n![Detailed dependency health](../../assets/enterprise/dependency-health-development.svg){ loading=lazy }\n\n</div>\n\n"+table(["Domain","Dependency","State","Evidence authority","Blocking","Root cause"],[[x["domain"],x["dependency"],x["state"],x["evidence_authority"],x["blocking"],x["root_cause"]] for x in deps]))
    outputs[DOC/"documentation-platform/page-types.md"]=doc_page("Documentation page types","Machine-enforced page anatomy contract.","development","# Page types\n\n## Domain page\nsummary · current state · capabilities · dependencies · evidence · known limitations · recovery · provenance\n\n## Runbook page\nsymptom · impact · likely causes · safe checks · recovery · verification · rollback · escalation · evidence\n\n## Threat model page\nboundary · assets · actors · entry points · allowed flows · forbidden flows · threats · controls · runtime evidence · residual risk · review status\n")
    task_details=[]
    for t in tasks:
        task_details.append(
    ...
    outputs[DOC/"search-synonyms.md"]=doc_page("Search aliases","Searchable alias terms mapped to canonical documentation destinations.","all","# Search aliases\n\nThis generated page gives MkDocs search deterministic synonym text without modifying Material internals.\n\n"+"\n".join(f"## {x['canonical']}\n\nAliases: {', '.join(x['synonyms'])}. Destinations: {', '.join(x['destinations'])}.\n" for x in synonyms))
