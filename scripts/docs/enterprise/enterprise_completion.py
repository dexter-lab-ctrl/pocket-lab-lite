...
def threat_model(root: Path, supply: dict[str,Any], release: dict[str,Any], deps: list[dict[str,Any]]) -> dict[str,Any]:
    ...
    controls=[
      ...
      {"id":"CTRL-HUMAN-SESSION-CSRF","description":"Human browser writes require an authenticated local-owner session plus a separate same-site CSRF proof; session credentials remain HttpOnly and hash-only at rest.","boundaries":["browser","control-api","durable-state"],"threats":["Spoofing","Tampering","Elevation of Privilege"],"implementation":["pocket-lab-final-structure/runtime/api_fastapi/deps.py","pocket-lab-final-structure/runtime/api_fastapi/services/lite_identity_auth.py","pocket-lab-final-structure/runtime/api_fastapi/routers/lite.py","src/lib/liteApi.js"],"source_refs":["pocket-lab-final-structure/runtime/api_fastapi/db/schema/0024_identity_rules_authorization.sql"],"tests":["tests/backend/test_lite_identity_rules_authorization.py"],"runtime_evidence":[],"status":"mitigation-source-derived","freshness":"source-current","owner":"FastAPI Identity/session boundary"},
      {"id":"CTRL-OPA-FAIL-CLOSED","description":"FastAPI preserves hard domain invariants, then requires a valid loopback OPA allow decision for registered protected actions before NATS/worker execution; unavailable, timed-out, malformed, or unregistered decisions fail closed.","boundaries":["control-api","messaging-execution"],"threats":["Spoofing","Tampering","Elevation of Privilege"],"implementation":["pocket-lab-final-structure/runtime/api_fastapi/services/lite_policy_opa.py","security/policies/opa/pocketlab/pocketlab.rego","pocket-lab-final-structure/runtime/api_fastapi/routers/lite.py"],"source_refs":["architecture/metadata/pocket-lab-architecture.json"],"tests":["tests/backend/test_lite_identity_rules_authorization.py","security/policies/opa/pocketlab/pocketlab_test.rego"],"runtime_evidence":[],"status":"mitigation-source-derived","freshness":"source-current","owner":"FastAPI Rules/OPA authorization boundary"},
